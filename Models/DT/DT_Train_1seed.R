...
seednumber <- c(2020); set.seed(seednumber)
...
DTmodel <- train(Immobilization ~ ., data = train, method = "rpart2", maxdepth = 10, tuneLength = 10, trControl = train.control); print(DTmodel)

# Cross validation
train.control <- trainControl(method = "repeatedcv", number = 5, repeats = 10)
trainCV <- Table2
DTmodelCV <- train(Immobilization ~ ., data = trainCV, method = "rpart2", maxdepth = 10, tuneLength = 10, trControl = train.control); print(DTmodelCV)
CV <- DTmodelCV$results

# Make prediction on test set 
predictions_test <- DTmodel %>% predict(test)
predictions_train <- DTmodel %>% predict(train)
predictions_CV <- DTmodel %>% predict(trainCV)
...
writeData(ExcelFile, sheet = 5, capture.output(DTmodel))
saveWorkbook(ExcelFile, paste(dirname(QSARFile),"/DT_",tools::file_path_sans_ext(basename(QSARFile)),".xlsx", sep = ""), overwrite = TRUE)
