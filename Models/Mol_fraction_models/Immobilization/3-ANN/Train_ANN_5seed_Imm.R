...
for (j in SeedTable[,i]) {
  ...
  ANNmodel <- train(Immobilization ~ ., data = train, method = "brnn", trControl = train.control); 
  print(ANNmodel)
  
  # Cross validation
  train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 3)
  trainCV <- train
  ANNmodelCV <- train(Immobilization ~ ., data = trainCV, method = "brnn", trControl = train.control); 
  print(ANNmodelCV)
