...
for (i in 1:1000) {
  ...
  ANNmodel <- train(pEC50 ~ ., data = train, method = "lm", trControl = train.control)
  print(ANNmodel)
...
saveWorkbook(ExcelFile, paste(QSARFolder,"/",tools::file_path_sans_ext(basename(QSARFile)),"_seed_MLR_brnn.xlsx", sep = ""), overwrite = TRUE)
