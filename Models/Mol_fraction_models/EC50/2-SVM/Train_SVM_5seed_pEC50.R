...
for (j in SeedTable[,i]) {
  ...
  SVMmodel <- train(pEC50 ~ ., data = train, method = "svmPoly", trControl = train.control); 
  print(SVMmodel)
  
  # Cross validation
  train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 3)
  trainCV <- train
  SVMmodelCV <- train(pEC50 ~ ., data = trainCV, method = "svmPoly", trControl = train.control); 
  print(SVMmodelCV)
  CV <- SVMmodelCV$results
  
  # Make prediction on test set 
  predictions_test <- SVMmodel %>% predict(test)
  predictions_train <- SVMmodel %>% predict(train)
  predictions_CV <- SVMmodel %>% predict(trainCV)
