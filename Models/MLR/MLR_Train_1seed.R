...
seednumber <- c(365); set.seed(seednumber)
...
MLRmodel <- train(Immobilization ~ ., data = train, method = "lm", trControl = train.control); print(MLRmodel)

# Cross validation
train.control <- trainControl(method = "repeatedcv", number = 5, repeats = 10)
trainCV <- Table2
MLRmodelCV <- train(Immobilization ~ ., data = trainCV, method = "lm",trControl = train.control); print(MLRmodelCV)
