...
for (j in SeedTable[,i]) {
  ...
  MLRmodel <- train(pEC50 ~ ., data = train, method = "lm", trControl = train.control); 
  print(MLRmodel)
  
  # Cross validation
  train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 3)
  trainCV <- train
  MLRmodelCV <- train(pEC50 ~ ., data = trainCV, method = "lm", trControl = train.control); 
  print(MLRmodelCV)
  CV <- MLRmodelCV$results
  
  # Make prediction on test set 
  predictions_test <- MLRmodel %>% predict(test)
  predictions_train <- MLRmodel %>% predict(train)
  predictions_CV <- MLRmodel %>% predict(trainCV)
