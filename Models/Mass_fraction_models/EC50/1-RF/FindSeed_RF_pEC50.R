...
for (i in 1:1000) {
  ...
  RFmodel <- train(pEC50 ~ ., data = train, method = "qrf", trControl = train.control)
  print(RFmodel)
  ...
  predictions_ext <- RFmodel %>% predict(Table1)
...
saveWorkbook(ExcelFile, paste(QSARFolder,"/",tools::file_path_sans_ext(basename(QSARFile)),"_seed_RF_qrf.xlsx", sep = ""), overwrite = TRUE)
