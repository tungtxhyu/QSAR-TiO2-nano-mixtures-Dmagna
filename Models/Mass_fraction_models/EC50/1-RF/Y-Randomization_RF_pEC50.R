...
# Building models
for (i in c(1:9)) {
  QSARdata <- read.xlsx(QSARFile, sheet = i, startRow = 1, colNames = TRUE,
                      rowNames = FALSE, detectDates = FALSE, skipEmptyRows = TRUE,
                      skipEmptyCols = TRUE, rows = NULL, cols = NULL, check.names = FALSE,
                      namedRegion = NULL, na.strings = "NA", fillMergedCells = FALSE)
...
for (j in SeedTable[,i]) {
  ...
  RFmodel <- train(pEC50 ~ ., data = train, method = "qrf", ntree = 500, trControl = train.control); 
  print(RFmodel)
  
  # Cross validation
  train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 3)
  trainCV <- Table1
  RFmodelCV <- train(pEC50 ~ ., data = trainCV, method = "qrf", ntree = 500, trControl = train.control); 
  print(RFmodelCV)
  ...
  TestPerfomance[1,2] <- R2(predictions_test,test$pEC50)
  TestPerfomance[1,3] <- R2(predictions_CV,trainCV$pEC50)
  TestPerfomance[1,4] <- R2(predictions_train,train$pEC50)
  TestPerfomance[1,5] <- RMSE(predictions_test,test$pEC50)
  TestPerfomance[1,6] <- RMSE(predictions_CV,trainCV$pEC50)
  TestPerfomance[1,7] <- RMSE(predictions_train,train$pEC50)
  TestPerfomance[1,8] <- MAE(predictions_test,test$pEC50)
  TestPerfomance[1,9] <- MAE(predictions_CV,trainCV$pEC50)
  TestPerfomance[1,10] <- MAE(predictions_train,train$pEC50)
  ...
  # Y-Randomization
  #-----------------------------------------------------------------------------
  Y_randomization <- data.frame("Iteration"       = c(1:10),
                                "R2_train"        = R2(predictions_train,train$pEC50),
                                "R2_train_Y_rand" = NA, 
                                "Z_Score" = NA)
  for (k in c(1:10)) {
    train_Y_rand <- subset(train, select = -c(`pEC50`))
    train_Y_rand$pEC50 <- sample(train$pEC50)
    RFmodel_Y_rand <- train(pEC50 ~ ., data = train, method = "qrf", ntree = 500, trControl = train.control)
    predictions_train_Y_rand <- RFmodel %>% predict(train_Y_rand)
    Y_randomization$R2_train_Y_rand[k] <- R2(predictions_train_Y_rand,train_Y_rand$pEC50)
  }
  Y_randomization$Z_Score[1] <- (Y_randomization$R2_train[1] - mean(Y_randomization$R2_train_Y_rand)) / sd(Y_randomization$R2_train_Y_rand)
  
  
  
  # Save output to excel file
  #-----------------------------------------------------------------------------
  Table3 <- train; Table3$Predict.pEC50 <- predictions_train
  Table4 <- test; Table4$Predict.pEC50 <- predictions_test
  ...
  addWorksheet(ExcelFile, "Y_randomization")
  writeData(ExcelFile, sheet = 6, Y_randomization)
