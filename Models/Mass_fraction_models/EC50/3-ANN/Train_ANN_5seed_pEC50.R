library(svDialogs) 
library(data.table)
library(openxlsx)
library(mlbench)
library(caret)
library(tools)
library(DT)
library(ggplot2)
library(car)
library(nortest)
library(tseries)
library(RcmdrMisc)
library(lmtest)
library(dplyr)
library(randomForest)

#-------------------------------------------------------------------------------
# Read excel data file 
QSARFile <- dlg_open(title = "Select excel file for Dmix", filters = dlg_filters[c("All"), ])$res
QSARFolder <- dirname(QSARFile)

SeedTableFile <- dlg_open(title = "Select excel file for Seed table", filters = dlg_filters[c("All"), ])$res
SeedTable <- read.xlsx(SeedTableFile, sheet = 1, startRow = 1, colNames = TRUE,
                       rowNames = FALSE, detectDates = FALSE, skipEmptyRows = TRUE,
                       skipEmptyCols = TRUE, rows = NULL, cols = NULL, check.names = FALSE,
                       namedRegion = NULL, na.strings = "NA", fillMergedCells = FALSE)
Savefolder <- dlg_dir(title = "Select folder to save data output:", filters = dlg_filters[c("All"), ])$res
#-------------------------------------------------------------------------------
# Building models
for (i in c(1:9)) {
  QSARdata <- read.xlsx(QSARFile, sheet = i, startRow = 1, colNames = TRUE,
                      rowNames = FALSE, detectDates = FALSE, skipEmptyRows = TRUE,
                      skipEmptyCols = TRUE, rows = NULL, cols = NULL, check.names = FALSE,
                      namedRegion = NULL, na.strings = "NA", fillMergedCells = FALSE)

  Table1 <- QSARdata[-c(2,52,53), -c(1,3)]

  TestPerfomance <- data.frame( Seed = 0,
                                R2_test = 0,
                                R2_CV = 0,
                                R2_train = 0,
                                RMSE_test = 0,
                                RMSE_CV = 0,
                                RMSE_train = 0,
                                MAE_test = 0,
                                MAE_CV = 0,
                                MAE_train = 0)
  
  
  
for (j in SeedTable[,i]) {
  set.seed(j)
  # Split data into train/test with ratio 7/3 of the sample size
  in_rows <- createDataPartition(y = Table1$pEC50, p = 0.66, list = FALSE)
  train <- rbind(Table1[in_rows, ],QSARdata[c(2,52,53),-c(1,3)])
  test <- Table1[-in_rows, ]
  
  # Training models
  train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 3)
  ANNmodel <- train(pEC50 ~ ., data = train, method = "brnn", trControl = train.control); 
  print(ANNmodel)
  
  # Cross validation
  train.control <- trainControl(method = "repeatedcv", number = 3, repeats = 3)
  trainCV <- train
  ANNmodelCV <- train(pEC50 ~ ., data = trainCV, method = "brnn", trControl = train.control); 
  print(ANNmodelCV)
  CV <- ANNmodelCV$results
  
  # Make prediction on test set 
  predictions_test <- ANNmodel %>% predict(test)
  predictions_train <- ANNmodel %>% predict(train)
  predictions_CV <- ANNmodel %>% predict(trainCV)
  
  # Compute the R2, RMSE and MAE for train, test and cross validation
  
  TestPerfomance[1,1] <- j
  TestPerfomance[1,2] <- R2(predictions_test,test$pEC50)
  TestPerfomance[1,3] <- R2(predictions_CV,trainCV$pEC50)
  TestPerfomance[1,4] <- R2(predictions_train,train$pEC50)
  TestPerfomance[1,5] <- RMSE(predictions_test,test$pEC50)
  TestPerfomance[1,6] <- RMSE(predictions_CV,trainCV$pEC50)
  TestPerfomance[1,7] <- RMSE(predictions_train,train$pEC50)
  TestPerfomance[1,8] <- MAE(predictions_test,test$pEC50)
  TestPerfomance[1,9] <- MAE(predictions_CV,trainCV$pEC50)
  TestPerfomance[1,10] <- MAE(predictions_train,train$pEC50)
  
  # Estimate Descriptor importance
  DescImportance <- data.frame(Descriptor = row.names(varImp(ANNmodel, scale=TRUE)$importance),
                               Value = varImp(ANNmodel, scale=TRUE)$importance)
  colnames(DescImportance) <- c("Descriptor", "Value")
  
  
  # Save output to excel file
  Table3 <- train; Table3$Predict.pEC50 <- predictions_train
  Table4 <- test; Table4$Predict.pEC50 <- predictions_test
  
  ExcelFile <- createWorkbook("TiO2")
  addWorksheet(ExcelFile, "train")
  writeData(ExcelFile, sheet = 1, Table3)
  addWorksheet(ExcelFile, "test")
  writeData(ExcelFile, sheet = 2, Table4)
  addWorksheet(ExcelFile, "Performance")
  writeData(ExcelFile, sheet = 3, TestPerfomance)
  addWorksheet(ExcelFile, "Desc_Importance")
  writeData(ExcelFile, sheet = 4, DescImportance)
  addWorksheet(ExcelFile, "ModelInfo")
  writeData(ExcelFile, sheet = 5, capture.output(ANNmodel))
  saveWorkbook(ExcelFile, paste(Savefolder,"/",tools::file_path_sans_ext(basename(QSARFile)),i,'_seed_',j,'.xlsx', sep=""), overwrite = TRUE)
}
}


